 ...
 *
 * L'appel à l'action n'y figure pas : la barre collante du bas le porte en
 * permanence, et le handoff refuse explicitement de le dupliquer.
 */
  ...
  return (
    <>
      <button
        ...
        aria-controls="rfe-menu-panneau"
      ...
      <div id="rfe-menu-panneau" className="rfe-menu__panneau" hidden={!ouvert}>
        <form action="/annuaire" className="rfe-recherche rfe-recherche--menu" role="search">
          <Loupe taille={17} />
          <input
            type="search"
            name="q"
        ...
        <Link href="/methodologie" onClick={() => setOuvert(false)}>
          {typo("Comment ça marche")}
        </Link>
        <Link href="/annuaire" onClick={() => setOuvert(false)}>
          Entreprises
        </Link>
        <Link href="/aide" onClick={() => setOuvert(false)}>
          Guides
        </Link>
      </div>
    </>
