 ...
 */
export function BandeauIndependance() {
  return (
    <div className="rf-bandeau-independance">
      <div className="rf-conteneur">
        <span>
...
export function Logo({ baseline = "Signalement des litiges de consommation" }: { baseline?: string }) {
  return (
    <Link href="/" className="rf-logo">
      ...
      <span>
        <span className="rf-logo__nom" style={{ display: "block" }}>
          Recours France
        </span>
        <span className="rf-logo__baseline" style={{ display: "block" }}>
          {baseline}
        </span>
      </span>
    </Link>
...
type EnteteProps = {
  baseline?: string;
  /** Affiche le champ de recherche central (annuaire, fiche entreprise). */
  recherche?: boolean;
...
export function Entete({ baseline, recherche, valeurRecherche, sansCta, navActive }: EnteteProps) {
  return (
    <header className="rf-entete">
      <div className="rf-conteneur">
        ...
        {/* Avec le champ de recherche, la navigation est réduite pour tenir sur
            une seule ligne à la largeur du conteneur. */}
        <nav className="rf-nav" aria-label="Navigation principale">
          {recherche ? null : navActive === "annuaire" ? (
...
export function FilAriane({ items }: { items: MailleFil[] }) {
  return (
        ...
        {items.map((item, i) => (
          <span key={`${item.libelle}-${i}`}>
            ...
            {item.href ? (
              ...
              <span className="rf-fil__actuel">{item.libelle}</span>
            )}
          </span>
        ))}
      </div>
...
export function PiedDePage({ complet = true }: { complet?: boolean }) {
  return (
    <footer className="rf-pied">
      {complet ? (
        <div className="rf-conteneur">
          <div className="rf-pied__colonnes">
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
                ...
                <span
                  style={{ fontSize: 18, fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.015em" }}
                >
                  Recours France
                </span>
              </div>
            ...
            <div>
              ...
              <div className="rf-pied__liens">
                <Link href="/methodologie#m2">Litige déclaré ou vérifié</Link>
                <Link href="/aide/droits">Vos droits de consommateur</Link>
                <Link href="/methodologie#m1">Origine de nos données</Link>
...
export function Page({
  ...
  return (
    <>
      <BandeauIndependance />
      <Entete {...entete} />
      {fil ? <FilAriane items={fil} /> : null}
      <main id="contenu">{children}</main>
      <PiedDePage complet={piedComplet} />
    </>
  );
}
